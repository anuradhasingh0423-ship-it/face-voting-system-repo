...
#     # Use DirectShow backend to avoid MSMF errors on Windows
...
#         ret, frame = cap.read()
#         if not ret:
#             # retry after a short delay to avoid flooding logs
#             cv2.waitKey(50)
#             continue
...
#         try:
#             # Pass the frame directly, no need to save it
#             temp_emb_obj = DeepFace.represent(
#                 img_path=frame, 
#                 model_name="VGG-Face", 
#                 enforce_detection=True,
#                 detector_backend='opencv' # Add this for better performance
#             )
...
# def choose_election():
#     # Get voter info from query parameters
#     mobile = request.args.get("mobile")
#     name = request.args.get("name")

#     # Store in session inside request context
#     if mobile and name:
...
#         # Reject if candidate not selected
#         if not candidate:
...
#             now.strftime("%H:%M:%S")
#         ]
#         df.to_csv(VOTES_FILE, index=False)  # SAVE immediately
...
#     print("Press CTRL+C in this terminal to stop the server for the next voter.")
